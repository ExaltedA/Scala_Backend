...
import akka.http.scaladsl.server.Directives.{as, complete, concat, entity, get, onSuccess, pathEnd, post, _}
...
class MyRouter(val todoRepository: TodoRepository)(implicit system: ActorSystem[_],  ex:ExecutionContext) extends  Router
  with  Directives{
  ...
  var listFilenames: Array[String] = Array()
  val uploadVideo = {
    path("video") {
      pathEnd {concat(
        get {
          complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, "<h1>Filenames</h1><br>" + listFilenames.mkString("Array(", ", ", ")")))
        },
        ...
        val allPartsF: Future[Map[String, Any]] = formData.parts.mapAsync[(String, Any)](1) {
          ...
          case b: BodyPart if b.name == "file" =>
            ...
            b.filename match {
              case Some(fn) =>
                listFilenames :+= fn
                //listFilenames= listFilenames :+ fn
              case None =>
            }
            println(listFilenames.mkString("Array(", ", ", ")"))

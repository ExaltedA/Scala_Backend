...
object ApiError {
  ...
  val emptyTitleField: ApiError = new ApiError(StatusCodes.BadRequest, "Fullname field must not be empty.")

  def todoNotFound(id: String): ApiError =
    new ApiError(StatusCodes.NotFound, s"The Address with id $id could not be found.")
}